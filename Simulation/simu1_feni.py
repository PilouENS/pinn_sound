...
# ── 5. Boucle A — sans obstacles ─────────────────────────────────────────────

all_data: list[np.ndarray] = []
print(f"\n── Boucle A : {n_samples} simulations sans obstacles ──")

for i in range(n_samples):
    cx, cy = rng.uniform(0.1, 0.9, size=2)
    print(f"  sample {i} : source=({cx:.3f}, {cy:.3f})")
    data, last_field_A = run(i, cx, cy, obstacle_mask=None)
    all_data.append(data)

if preview:
    fig, ax = plt.subplots(figsize=(5, 4))
    im = ax.imshow(last_field_A, origin="lower", extent=[0,1,0,1],
                   cmap="coolwarm", aspect="equal")
    ax.set_title(f"Sans obstacles — sample {n_samples-1} | t≈{n_steps*dt:.4f}s")
    ax.set_xlabel("x") ; ax.set_ylabel("y")
    fig.colorbar(im, ax=ax, label="Pression")
    plt.tight_layout()
    plt.savefig("preview_no_obstacles.png", dpi=150)
    plt.close(fig)
    print("  → preview_no_obstacles.png sauvegardé")

np.save("pinn_ground_truth.npy", np.vstack(all_data))
print(f"  Dataset A sauvegardé : {np.vstack(all_data).shape}")

# ── 6. Boucle B — avec obstacles aléatoires ───────────────────────────────────

all_data_obs: list[np.ndarray] = []
print(f"\n── Boucle B : {n_obs_samples} simulations avec {n_obstacles} obstacles ──")

for i in range(n_obs_samples):
    cx, cy = rng.uniform(0.1, 0.9, size=2)
    mask   = make_obstacle_mask(n_obstacles, rng)
    print(f"  sample {i} : source=({cx:.3f}, {cy:.3f}) | {mask.sum()} cellules masquées")
    data, last_field_B = run(i, cx, cy, obstacle_mask=mask)
    all_data_obs.append(data)

    if preview:
        fig, axes = plt.subplots(1, 2, figsize=(10, 4))

        # Champ de pression
        im = axes[0].imshow(last_field_B, origin="lower", extent=[0,1,0,1],
                            cmap="coolwarm", aspect="equal")
        axes[0].set_title(f"Pression — sample {i}")
        axes[0].set_xlabel("x") ; axes[0].set_ylabel("y")
        fig.colorbar(im, ax=axes[0], label="Pression")

        # Masque des obstacles
        axes[1].imshow(mask.T, origin="lower", extent=[0,1,0,1],
                       cmap="Greys", aspect="equal")
        axes[1].set_title(f"Obstacles — sample {i}")
        axes[1].set_xlabel("x") ; axes[1].set_ylabel("y")
        # Marquer la source
        axes[1].plot(cx, cy, "r*", markersize=12, label="source")
        axes[1].legend()

        plt.suptitle(f"Simulation avec obstacles — sample {i}", fontsize=11)
        plt.tight_layout()
        fname = f"preview_obstacles_sample{i}.png"
        plt.savefig(fname, dpi=150)
        plt.close(fig)
        print(f"  → {fname} sauvegardé")

np.save("pinn_ground_truth_obstacles.npy", np.vstack(all_data_obs))
print(f"  Dataset B sauvegardé : {np.vstack(all_data_obs).shape}")
